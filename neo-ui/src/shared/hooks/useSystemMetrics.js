...
export function useSystemMetrics() {
    ...
    const sendMessage = useCallback((type, payload) => {
        return new Promise((resolve) => {
            chrome.runtime.sendMessage({ type, source: 'dashboard', payload, timestamp: Date.now() }, (response) => {
    ...
    const poll = useCallback(async () => {
        if (!mountedRef.current)
            return;
        try {
            const response = (await sendMessage('METRICS_POLL'));
            if (!mountedRef.current)
                return;
            if (response.success && response.data) {
                const { neo_system_metrics, neo_metrics_history, neo_bridge_connected, neo_panic_state } = response.data;
...
//# sourceMappingURL=useSystemMetrics.js.map