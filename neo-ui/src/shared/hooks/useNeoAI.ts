...
export interface UseNeoAIReturn {
  messages: AIMessage[];
  isThinking: boolean;
  isWarming: boolean;
  isConnected: boolean;
...
export function useNeoAI(): UseNeoAIReturn {
  const [messages, setMessages]       = useState<AIMessage[]>([]);
  const [isThinking, setThinking]     = useState(false);
  const [isWarming, setWarming]       = useState(false);
  const [isConnected, setConnected]   = useState(false);
  const [provider, setProviderState]  = useState<AIProvider>('ollama');
  const [model, setModelState]        = useState('llama3.2:3b');
  const [availableModels, setModels]  = useState<OllamaModel[]>([]);
  ...
  useEffect(() => {
    ...
    chrome.storage.local.get(
      ['neo_ai_model', 'neo_ai_connected', 'neo_ai_warming', 'neo_ai_provider', 'neo_ai_keys'],
      (result) => {
        if (!mountedRef.current) return;
        if (result.neo_ai_model)    setModelState(result.neo_ai_model);
        if (result.neo_ai_provider) setProviderState(result.neo_ai_provider as AIProvider);
        if (result.neo_ai_keys)     setApiKeys(result.neo_ai_keys);
    ...
    const listener = (changes: Record<string, chrome.storage.StorageChange>) => {
      if (changes.neo_ai_thinking  !== undefined) setThinking(changes.neo_ai_thinking.newValue ?? false);
      if (changes.neo_ai_warming   !== undefined) setWarming(changes.neo_ai_warming.newValue ?? false);
      if (changes.neo_ai_connected !== undefined) setConnected(changes.neo_ai_connected.newValue ?? false);
      if (changes.neo_ai_model     !== undefined) setModelState(changes.neo_ai_model.newValue ?? 'llama3.2:3b');
      if (changes.neo_ai_provider  !== undefined) setProviderState(changes.neo_ai_provider.newValue ?? 'ollama');
      if (changes.neo_ai_keys      !== undefined) setApiKeys(changes.neo_ai_keys.newValue ?? {});
    };
  ...
  const checkStatus = useCallback(async () => {
    const resp = (await sendMessage('AI_STATUS')) as {
      success: boolean;
      data?: { available: boolean; models: OllamaModel[]; provider: string };
    };
    if (resp.success && resp.data) {
      setConnected(resp.data.available);
      if (resp.data.models?.length) setModels(resp.data.models);
    } else {
      setConnected(false);
    }
  }, [sendMessage]);

  const warmup = useCallback(async (modelName?: string) => {
    const target = modelName ?? model;
    if (mountedRef.current) setWarming(true);
    try {
  ...
  useEffect(() => {
    checkStatus().then(async () => {
      const stored = await new Promise<{ neo_ai_provider?: string }>((res) =>
        chrome.storage.local.get('neo_ai_provider', res)
      );
  ...
  const setProvider = useCallback((p: AIProvider) => {
    ...
    if (p === 'ollama') warmup('llama3.2:3b');
  }, [sendMessage, warmup]);

  const setModel = useCallback((name: string) => {
    setModelState(name);
    sendMessage('AI_MODEL_SET', { model: name });
    if (provider === 'ollama') warmup(name);
  }, [provider, sendMessage, warmup]);

  const setApiKey = useCallback((prov: AIProvider, key: string) => {
    ...
    chrome.storage.local.get('neo_ai_keys', (stored) => {
      ...
      const updates: Record<string, unknown> = { neo_ai_keys: merged };
    ...
    sendMessage('AI_KEY_SET', { provider: prov, key }).catch(() => {/* ignore */});
  }, [sendMessage]);

  const sendQuery = useCallback(async (prompt: string) => {
    if (!prompt.trim() || isThinking || isWarming) return;
    ...
    try {
      const resp = (await sendMessage('AI_QUERY', { prompt, model, provider })) as {
        success: boolean;
        data?: { response: string; model: string; provider: string; eval_count: number; duration_ms: number };
        error?: string;
      };
      ...
      if (mountedRef.current) {
        setMessages((prev) => [...prev, {
          ...
          provider:    (resp.data?.provider ?? provider) as AIProvider,
